...
// Helper function to resolve document references
const resolveDocumentReferences = async (docRefs) => {
  ...
  try {
    ...
    const promises = validRefs.map((ref) => ref.get())
...
const createPlayerDocumentReference = (playerData, isActive = true) => {
  ...
  return db.collection("processedPlayers").doc("players").collection(collection).doc(playerId)
}
...
exports.onPlayerStatusChange = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (before.gameStatus !== "Concluded" && after.gameStatus === "Concluded") {
      ...
      console.log(`Moved players/active/${docId} → players/concluded/${docId}`)

      // Update any document references pointing to this player
      await updatePlayerReferences(docId, true) // true = moved from active to concluded
    }
  })

// Helper function to update document references when players move from active to concluded
const updatePlayerReferences = async (playerId, movedToConcluded) => {
  try {
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      // Update active bets - move references from active to concluded collection
      if (movedToConcluded) {
...
exports.onActiveBetWrite = functions.firestore
  ...
  .onWrite(async (change, ctx) => {
    ...
    if (wasDeleted || statusChanged) {
      ...
      // If bet has document references in picks, convert them to concluded references
      ...
      const hist = db.collection("users").doc(userId).collection("betHistory").doc(year).collection(month).doc(betId)
      ...
      console.log(`Moved users/${userId}/active/${betId} → users/${userId}/betHistory/${year}/${month}/${betId}`)
    }
  })
...
exports.onUserPicksUpdate = functions.firestore.document("users/{userId}").onUpdate(async (change, ctx) => {
  ...
  for (const pick of after) {
    if (pick && typeof pick.get === "function") {
      // This is a document reference, check if the referenced document is concluded
      try {
      ...
      try {
        ...
        if (picks.length > 0 && picks[0] && typeof picks[0].get !== "function") {
          ...
          const pickReferences = picks.map((pick) => {
            const playerId = pick.id || `${pick.name?.toLowerCase().replace(/\s+/g, "_")}_${pick.threshold}`
        ...
        for (const betDoc of activeBetsSnapshot.docs) {
          ...
          if (betPicks.length > 0 && betPicks[0] && typeof betPicks[0].get !== "function") {
            const pickReferences = betPicks.map((pick) => {
              const playerId = pick.id || `${pick.playerName?.toLowerCase().replace(/\s+/g, "_")}_${pick.threshold}`
              return db.collection("processedPlayers").doc("players").collection("active").doc(playerId)
            })
