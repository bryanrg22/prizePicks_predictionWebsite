...
exports.onPlayerStatusChange = functions.firestore
    ...
    .onUpdate(async (change, context) => {
      ...
      if (before.gameStatus !== "Concluded" && after.gameStatus ===
        "Concluded") {
...
// Only create betHistory for bets with terminal status
exports.onActiveBetWrite = functions.firestore
    ...
    .onWrite(async (change, ctx) => {
      ...
      // Check if  bet has a terminal status
      const hasTerminalStatus = (betData) => {
        return betData && betData.status &&
        ["Concluded", "Completed", "Won", "Lost"]
            .includes(betData.status);
      };

      // Only create betHistory if:
      // 1. Status changed to terminal state, OR
      // 2. Bet was deleted AND it had a terminal status (game concluded)
      const statusChangedToTerminal = before && after &&
    before.status !== after.status && hasTerminalStatus(after);

      const deletedWithTerminalStatus = wasDeleted && hasTerminalStatus(before);

      // 🚫 CRITICAL FIX: Only create betHistory for concluded/completed bets
      if (statusChangedToTerminal || deletedWithTerminalStatus) {
        const betData = wasDeleted ? before : after;

        console.log(`📊 Creating betHistory for ${betId} - Status: 
        ...
        typeof updatedPicks[0].get === "function") {
        ...
        const hist = db.collection("users").doc(userId)
      ...
      // This is a manual cancellation - log it but don't create history
        console.log(`🚫 Manual cancellation detected for bet 
        ${betId} - NO betHistory created`);
      }
    });

// 3) Clean up user picks when games conclude (with document reference handling)
exports.onUserPicksUpdate = functions.firestore.document("users/{userId}")
      ...
      for (const pick of after) {
        if (pick && typeof pick.get === "function") {
          ...
          try {
            ...
            if (pickDoc.exists && pickDoc.data().gameStatus !==
        "Concluded") {
          ...
          try {
            ...
            for (const betDoc of activeBetsSnapshot.docs) {
              ...
              if (betPicks.length > 0 && betPicks[0] && typeof betPicks[0]
                ...
                console
                    .log(`Migrated active bet ${betDoc.id} for user ${userId}`);
              }
            }
