...
const hasTerminalStatus = (betData) => {
  return betData && betData.status &&
  ["Concluded", "Completed", "Won", "Lost"].includes(betData.status);
};
...
const updatePlayerReferences = async (playerId, movedToConcluded) => {
  try {
    console.log(`Updating references for player ${playerId}, 
      moved to concluded: ${movedToConcluded}`);
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      if (movedToConcluded) {
        ...
        const activeBetsSnapshot = await db.collection("users")
            .doc(userId).collection("activeBets").get();
...
exports.onPlayerStatusChange = functions
    .region("us-west2")
    .firestore
    ...
    .onUpdate(async (change, context) => {
      ...
      if (before.gameStatus !== "Concluded" &&
        ...
        try {
          console.log(`Player ${docId} game concluded, moving 
            to concluded collection`);
...
exports.onActiveBetWrite = functions
    .region("us-west2")
    .firestore
    ...
    .onWrite(async (change, ctx) => {
      ...
      const statusChangedToTerminal = before && after && before.status !==
    after.status && hasTerminalStatus(after);
      ...
      if (statusChangedToTerminal || deletedWithTerminalStatus) {
        try {
          ...
          console.log(`Creating betHistory for bet ${betId} - Status: 
          ${betData.status || "CONCLUDED"}`);

          // Convert any active references to concluded references
          let updatedPicks = betData.picks || [];
          if (updatedPicks.length > 0 && updatedPicks[0] &&
            typeof updatedPicks[0]
                .get === "function") {
          ...
          const historyRef = db.collection("users").doc(userId)
              .collection("betHistory").doc(betId);
      ...
      // Manual cancellation - log but don't create history
        console.log(`🚫 Manual cancellation detected for bet ${betId} 
        - NO betHistory created`);
      }
    });

// 3) Clean up user picks when games conclude
exports.onUserPicksUpdate = functions
    .region("us-west2") // ← pin it here
    .firestore
    ...
    .onUpdate(async (change, ctx) => {
      ...
      try {
          ...
          if (pick && typeof pick.get === "function") {
            ...
            try {
              ...
              if (pickDoc.exists && pickDoc.data().gameStatus !== "Concluded") {
                ...
                console.log(`Removing concluded pick reference for user 
              ${ctx.params.userId}`);
              }
            } catch (error) {
            ...
            needsUpdate = true;
            console.log(`Removing concluded legacy pick for user 
          ${ctx.params.userId}`);
          }
        }
        ...
        if (needsUpdate) {
          ...
          console.log(`Cleaned up concluded picks for user 
        ${ctx.params.userId}`);
        }
      } catch (error) {
        console.error(`Error cleaning up picks for user 
      ${ctx.params.userId}:`, error);
      }
    });

// 4) Migration function to convert existing data to document references
exports.migrateToDocumentReferences = functions
    .region("us-west2")
    .https
    .onRequest(async (req, res) => {
      try {
          ...
          try {
            ...
            for (const betDoc of activeBetsSnapshot.docs) {
              ...
              if (betPicks.length > 0 && betPicks[0] && typeof betPicks[0]
                ...
                console.log(`Migrated active bet ${betDoc.id} for user 
              ${userId}`);
              }
            }
        ...
        res.json({
          success: true,
          message: `Successfully migrated ${migratedUsers} users to 
      document references`,
