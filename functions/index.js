...
// FIXED: Only create betHistory for bets with terminal status, NOT manual cancellations
exports.onActiveBetWrite = functions.firestore
  ...
  .onWrite(async (change, ctx) => {
    ...
    // Check if the bet has a terminal status indicating it was completed/concluded
    ...
    // NOT for manual cancellations (which are deletions without terminal status)
    if (statusChangedToTerminal || deletedWithTerminalStatus) {
      const betData = wasDeleted ? before : after

      console.log(`📊 Creating betHistory for ${betId} - Status: ${betData.status || "CONCLUDED"}`)
      ...
      try {
        ...
        if (picks.length > 0 && picks[0] && typeof picks[0].get !== "function") {
          ...
          const pickReferences = picks.map((pick) => {
            ...
            const playerId = pick.id || `${pickName.toLowerCase().replace(/\s+/g, "_")}_${pick.threshold}`
        ...
        for (const betDoc of activeBetsSnapshot.docs) {
          ...
          if (betPicks.length > 0 && betPicks[0] && typeof betPicks[0].get !== "function") {
            const pickReferences = betPicks.map((pick) => {
              ...
              const playerId = pick.id || `${playerName.toLowerCase().replace(/\s+/g, "_")}_${pick.threshold}`
              return db.collection("processedPlayers").doc("players").collection("active").doc(playerId)
            })
